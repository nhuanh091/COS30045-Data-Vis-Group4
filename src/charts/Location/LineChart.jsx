// src/charts/Location/LineChart.jsx
import { useRef, useEffect, useState } from 'react'
import * as d3 from 'd3'
import { Box, Typography } from '@mui/material'
import { useStore } from '../../store/useStore'

const MARGIN = { top: 40, right: 36, bottom: 44, left: 52 }
const COLORS = ['#61196E', '#E99E1C', '#7658B2', '#BF6BA1', '#852501']

/**
 * Line chart for locations over time.
 * Reads filters.locationGroup from store — fades non-selected locations.
 * Click on a legend item or line to toggle locationGroup filter.
 *
 * @param {Array}    data     - [{ month: "2023-01", "Metropolitan": number, "Regional": number, ... }]
...
function LineChart({ data = [], onReset }) {
  ...
  useEffect(() => {
    ...
    // Get locations from data keys (excluding 'month')
    const locations = Object.keys(data[0] || {}).filter(key => key !== 'month')
    ...
    const y = d3.scaleLinear()
      .domain([0, d3.max(data.flatMap(d => locations.map(loc => d[loc] || 0))) * 1.2])
      .nice()
      .range([innerH, 0])
    ...
    // Draw lines and dots for each location
    locations.forEach((loc, i) => {
      const isSelected = !hasLocationGroupFilter || loc === selectedLocationGroup
      ...
      g.append('path')
        .datum(data)
        .attr('fill', 'none')
        .attr('stroke', color(loc))
        .attr('stroke-width', isSelected ? 3 : 2)
        .attr('opacity', lineOpacity)
        .attr('d', lineGen)
        .style('cursor', 'pointer')
        .on('click', () => {
          if (filters.locationGroup === loc) {
            setFilter('locationGroup', null)
          } else {
            setFilter('locationGroup', loc)
          }
        })
    ...
    locations.forEach((loc) => {
      ...
      // Approximate text length: 6px per character + icon + padding
      const estTextWidth = loc.length * 6
      const itemWidth = 24 + estTextWidth

      // Wrap if exceeding innerW
      if (legendX + itemWidth > innerW && legendX > 0) {
        legendX = 0
        legendY += 16
      }
      ...
      legendItem.append('line')
        ...
        .attr('stroke', color(loc)).attr('stroke-width', 2.5)
      legendItem.append('circle')
        .attr('cx', 6).attr('cy', 6).attr('r', 3)
        .attr('fill', color(loc))
    ...
    g.append('rect')
      ...
      .on('mousemove', function(event) {
        ...
        const tooltipContent = `<strong>${d3.timeFormat('%B %Y')(new Date(d.month + '-01'))}</strong><br/>` +
          locations.map(loc => `${loc}: <strong>${(d[loc] || 0).toLocaleString()}</strong>`).join('<br/>')
  ...
  }, [data, width, filters.locationGroup, filters.year, filters.month])
